class TopDownHeapSorter implements Sorter {
    public String getName() {
        return "Top Down Heap Sort";
    }

    public SortResult sort(int[] array) {
        // This one uses my own HeapArray class, which
        // extends abstract data type ADTHeap
        HeapArray heap = new HeapArray();

        // Build the heap (top-down for this sorter)
        heap.buildHeapTopDown(array);

        // Now remove the elements from the heap one by one and put them back into the
        // original array in order
        for (int j = array.length - 1; j >= 0; j--) {
            // Put them in descending order from right to left, 
            // i.e, increasing order from left to right
            array[j] = heap.remove();
        }

        return new SortResult(array, heap.getNumComparisons(), heap.getNumExchanges());
    }
}